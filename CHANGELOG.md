...
- **Sender GUI: keyboard device is now a dropdown of detected devices.** The "Keyboard device" field was a free-text entry that required hand-typing a full `/dev/input/by-id/...` path, which was error-prone and gave no hint when multiple keyboards were present. It is now a dropdown populated from `/dev/input/by-id` with friendly labels (model name plus resolved event node, e.g. `Vendor Model KB  (event11)`), plus a leading `(auto-detect)` entry that maps to an empty value so the sender falls back to its own detection. A configured device that is currently unplugged is still listed and preserved rather than silently dropped. The **Detect Keyboard** button rescans and repopulates the list. This makes it straightforward to pick the correct keyboard when a wireless mouse receiver also exposes a phantom `-event-kbd` node that the sender's alphabetical auto-detection would otherwise grab.
